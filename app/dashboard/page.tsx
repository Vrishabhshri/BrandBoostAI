...
export default function OverviewPage() {
  ...
  const { metrics, recentPosts, socialMediaPerformance, recentPostsSection, lockStatus } = overview;
    ...
    <div className="flex min-h-screen flex-col bg-zinc-900">
      ...
      <main className="flex-1 p-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-semibold text-white">{overview.title}</h1>
          <button 
            className="flex items-center gap-2 rounded-full border border-zinc-700 px-3 py-1"
            aria-label={lockStatus.text}
          >
            <Lock className="h-4 w-4 text-zinc-400" />
            <span className="text-sm text-zinc-400">{lockStatus.text}</span>
          </button>
        </div>
        ...
        <div className="mt-8 grid grid-cols-2 gap-6">
          <div className="rounded-lg bg-zinc-800 p-4">
            <h2 className="text-lg font-medium text-white">{socialMediaPerformance.title}</h2>
            <p className="text-sm text-zinc-400">{socialMediaPerformance.subtitle}</p>
